  ...
  function addLegend(plot, options) {
    if (options.legend.data) {
      elements = $(options.legend.data).collect(function(i) {
        ...
        var colorBox = '<div class="color-box box-rounded" style="background-color:' + optionResolver(options.legend.color) + '"></div>';
        var label = '<div style="color:' + optionResolver(options.legend.color) + '">' + optionResolver(options.legend.label) + '</div>';

        return "<td>" + colorBox + "</td><td>" + label + "</td>";
      });

      $('<table class="legend"><tr>' + elements.reverse().join("") + '</tr></table>').css({
        position: 'absolute',
        top:  '0px',
        left: plot.width + 'px'
